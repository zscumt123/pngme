...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk, PngError> {
        let s = self
            ...
            .ok_or(PngError::ChunkRemoveError)?;
        Ok(self.chunks.remove(s))
    }
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        let mut t = self
            ...
            .flatten()
            .collect::<Vec<_>>();
        let mut s = self.signature.to_vec();
        s.append(&mut t);
        s
    }
}
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self, Self::Error> {
        ...
        if header != Png::STANDARD_HEADER {
            return Err(PngError::CreatePngError);
        }
        let mut chunks: Vec<Chunk> = vec![];
        let mut chunk_len_flag = 0;
        let mut i = 0;
        let extra = &value[8..];
        ...
        while i < extra_len {
            if chunk_len_flag < 4 {
                ...
                let num: [u8; 4] = (&extra[i - 4..i]).try_into().unwrap();
                let chunk_len = u32::from_be_bytes(num);
                let last = i + 4 + (chunk_len as usize) + 4;
                if last > extra_len {
                    return Err(PngError::CreatePngError);
                }
                let c: Chunk = (&extra[i - 4..last]).try_into().unwrap();
                chunks.push(c);
                chunk_len_flag = 0;
                i = last
            }
        ...
        Ok(Self {
            signature: header.try_into().unwrap(),
            chunks,
        })
    }
}
